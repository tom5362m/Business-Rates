...
load_data<-function(data_set_1, data_set_2) {
  if(is.null(data_set_1)==FALSE & is.null(data_set_2)==FALSE){
    rbind(data_set_1,data_set_2)
    }
  if(is.null(data_set_1)==FALSE){
  raw_data<-read_excel(data_set_1)
  }
  else{
    raw_data<-read_excel(data_set_2)
  }
  raw_data <-raw_data%>%
...
cases<-cases%>%
  mutate(to=ifelse(Relationship=="Child",
                   cases$id, cases$to))|>#for child cases putting the AK in the to column 
  mutate(from=ifelse(Relationship=="Parent"
                     ,cases$id, cases$from))#for parent cases putting the AK in the from column
...
final_cases_parent<-left_join(cases,parent_values,by="Case",relationship = "many-to-many")#binds the parent data to the cases dataset
final_cases_child<-left_join(cases,child_values,by="Case",relationship = "many-to-many")#binds the child data to the cases data set

final_cases_together<-rbind(final_cases_child,final_cases_parent)|>
  rename(Relationship="Relationship.x")#binds the 2 data sets together
...
final_cases<-final_cases_together%>%
  ...
  mutate(to=ifelse(Relationship=="Parent" & Relationship.y=="Child"&from!=0&to==0,
                   id.y,to))#creating the matching algorithm for cases to create the from and to column

link_data<-data.frame(final_cases$from,final_cases$to)|>
  rename(from="final_cases.from",to="final_cases.to")#the reconstitution data frame
...
address_edge_creation<-function(address_data){
  ...
  filter("Change Reason"=="ADDRESS CHANGE & REVIEW OF ASSESSMENT")|>#creating the data set for address changes
