...
table1 <- NA
data_t<-NA
for (i in seq_along(data_t)) {
    table1[i] <- kable(data.frame(AK = data_t[[i]]$title,
                                   `RV` = data_t[[i]]$RV,
                                   `Address` = data_t[[i]]$Address,
                                  `Relationship` = data_t[[i]]$Relationship,
                                  `List Alteration Date` = data_t[[i]]$Alteration,
                                  `Effective Date` = data_t[[i]]$effective_date), format='html', table.attr='class="flat-table"')
    
  }
content <- str_glue("<em>{data$address}</em>{table1}")
return(content)
